...
python <skill_dir>/scripts/render_tiles.py tiles drawing.pdf -o tiles/ \
    --annotate geometry.json --rows 4 --cols 4 --dpi 350
```
...
each captioned with its label ID and the raw OCR. View each sheet and transcribe what you
actually see, citing IDs:
...
A worked example is `work/mod_linx/schematic_extraction/author_circuit_logic.py`.
...
Everything goes in one directory per drawing — nothing in this skill has a hardcoded
destination, so state the output directory explicitly and keep drawings separated:

```
<work>/<machine>/schematic_extraction/<DRAWING_NUMBER>/
...
**For indexing:**
> Build the LightRAG custom KG from circuit_logic.json, validate it, and index it into the
> mod_linx working directory. Then query in hybrid mode to check the netlist survived.
